...
agent_prompt_template = ChatPromptTemplate.from_messages([
    ("system", """You are a specialized biomedical research assistant. Your goal is to conduct a literature review on a given topic.

You will proceed through the following stages:
1.  **search**: Use the `pubmed_search` and `arxiv_search` tools to find relevant papers...
2.  **process**: For promising papers found in the search that have a direct PDF URL (e.g., from arXiv), use the `download_and_extract_pdf` tool...
3.  **synthesize**: Once you have gathered sufficient abstracts and processed relevant full texts..., you will stop using tools. You will then analyze the collected abstracts and full texts to write a comprehensive summary and identify research gaps.
...
- After using search tools, review the results. If there are relevant arXiv papers, consider calling `download_and_extract_pdf` for them.
- Once you believe you have enough information or cannot get more full-text, provide your final synthesis without calling any more tools."""),
...
def agent_node(state: ResearchState) -> dict:
                ...
                if isinstance(tool_output_data, list):
                        ...
                        if isinstance(item_data, dict):
                            try:
                                ...
                                if paper.source != "Error":
                                    if paper.paper_id not in [p.paper_id for p in new_search_results]:
                    ...
                    try:
                        ...
                        if paper.has_full_text and paper.source != "Error":
                            if paper.paper_id not in [p.paper_id for p in new_processed_papers]:
    ...
    if getattr(result_message, 'tool_calls', None):
        if any(tc.get('function', {}).get('name') in ["pubmed_search", "arxiv_search"] for tc in result_message.tool_calls):
            new_current_stage = "search"
        elif any(tc.get('function', {}).get('name') == "download_and_extract_pdf" for tc in result_message.tool_calls):
...
def synthesize_node(state: ResearchState) -> dict:
    ...
    for paper in state["processed_papers"]:
        if paper.url not in retrieved_urls:
            processed_abstracts += f"\n\nTitle: {paper.title}\nAbstract: {paper.abstract}\nURL: {paper.url}"
    if processed_abstracts:
        context_text += f"\n\n--- Processed Paper Abstracts (not in full-text RAG) ---\n{processed_abstracts}"

    synthesis_prompt = ChatPromptTemplate.from_template("""
Based on the following research context, please provide a comprehensive summary of the findings regarding the user's query: "{query}".
...
""")
    ...
    result = synthesis_chain.invoke({
        ...
        "context": context_text if context_text else "No relevant full-text documents were processed, and no relevant abstracts were provided from processed papers."
    })
